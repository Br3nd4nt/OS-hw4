...
void *handle_client(void *arg) {
    ...
    while (1) {
        ...
        if (client_type == -1) {
            // Register new client
            // client_type = (client_count % 3) + 1;
            const char *tmp = &buffer[len - 1];
            client_type = atoi(tmp);
            clients[client_count].addr = client_addr;
            clients[client_count].addr_len = addr_len;
            clients[client_count].type = client_type;
            client_count++;

            // Send acknowledgment to the new client
            char ack[BUF_SIZE];
            snprintf(ack, BUF_SIZE, "Registered as section %d", client_type);
            sendto(server_socket, ack, strlen(ack), 0, (struct sockaddr*)&client_addr, addr_len);
        } else {
            // Forward message to the next section
            if (client_type == 1) {
                for (int i = 0; i < client_count; i++) {
                    if (clients[i].type == 2) {
